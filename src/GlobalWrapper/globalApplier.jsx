...
import {
  Zap, Undo2, HeartCrack, Sword, BookOpen, ShieldCheck, XCircle, Skull,
  Bolt, Bomb, Angry, Shield, Syringe, Lock, Star, Repeat, Crosshair,
  Flower2, Droplets, FlaskConical, Snowflake, GitBranch, Flame, Biohazard,
  Gem, Grape, CircuitBoard, Asterisk, Wind, Anchor, EyeOff, EarOff,
  Hand, Utensils, Waves,
} from 'lucide-react';
...
const iconComponents = {
    Zap, Undo2, HeartCrack, Sword, BookOpen, ShieldCheck, XCircle, Skull, Bolt,
    Bomb, Angry, Shield, Syringe, Lock, Star, Repeat, Crosshair, Flower2, Droplets,
    FlaskConical, Snowflake, GitBranch, Flame, Biohazard, Gem, Grape, CircuitBoard,
    Asterisk, Wind, Anchor, EyeOff, EarOff, Hand, Utensils, Waves,
};
...
export default function GlobalKeywordApplier({ children }) {
    ...
    const highlightNode = useCallback((node) => {
        ...
        while ((currentNode = treeWalker.nextNode())) {
            ...
            if (parent && (parent.tagName === 'SCRIPT' || parent.tagName === 'STYLE' || parent.closest('[data-no-highlight]') || parent.closest('.keyword-highlight'))) {
                continue;
            }
        ...
        nodesToProcess.forEach(textNode => {
            ...
            parts.forEach((part) => {
                if (!part) return;
                const keyword = keywordData.find(k => k.term.toLowerCase() === part.toLowerCase());

                if (keyword) {
                    const IconComponent = iconComponents[keyword.iconName];
                    const iconHtml = IconComponent ? ReactDOMServer.renderToString(<IconComponent size={16} className="inline-block" />) : '';
